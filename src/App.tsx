...
import LoadingDebug from './components/LoadingDebug'; // Import the debug component
...
function App() {
  ...
  const [showDebugger, setShowDebugger] = useState(false); // State for showing debugger
  const [loadingTimeout, setLoadingTimeout] = useState(false); // State to detect loading timeouts
  ...
  useEffect(() => {
    // If loading takes more than 10 seconds, show debugger
  ...
  // Show debugger if loading takes too long
  useEffect(() => {
    if (loadingTimeout) {
  ...
  const MainContent = () => {
    ...
    // Show debugger if explicitly enabled or if loading takes too long
    if (showDebugger || (loading && loadingTimeout)) {
      return <LoadingDebug />;
    }
