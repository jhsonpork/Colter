...
const IdeaToCompany: React.FC<IdeaToCompanyProps> = ({ onUpgradeClick, hasUsedFreeTrial }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError('There was an error generating your company plan. Please try again.');
    } finally {
              ...
              >
                {isGenerating ? (
                  <>
                    ...
                    <span>Building Company Blueprint...</span>
                  </>
                ) : hasUsedFreeTrial ? (
                  <>
          ...
          <div className="space-y-8 animate-fade-in">
              ...
              <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6">
                ...
                <div className="space-y-3">
                  <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                    <h5 className="text-white font-semibold mb-1">Target Market</h5>
                    <p className="text-gray-300 text-sm">{companyResult.marketAnalysis.targetMarket}</p>
                  </div>
                  <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                    <h5 className="text-white font-semibold mb-1">Market Size</h5>
                    <p className="text-gray-300 text-sm">{companyResult.marketAnalysis.marketSize}</p>
                  </div>
                  <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                    <h5 className="text-white font-semibold mb-1">Unique Value Proposition</h5>
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <button
                  onClick={() => handleCopy(companyResult.revenueStreams.join('\n'))}
                  ...
                  <tbody>
                    {companyResult.startupCosts.map((cost, index) => (
                      <tr key={index} className="bg-gray-800/30 border-b border-gray-700/30">
                        ...
                        <td className="px-4 py-3 text-green-400">{cost.estimate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6">
              <div className="flex justify-between items-start mb-4">
                <h4 className="text-blue-400 font-bold text-lg">🚀 Launch Plan</h4>
                <button
                  onClick={() => handleCopy(companyResult.launchPlan.join('\n'))}
                  className="p-2 text-gray-400 hover:text-white transition-colors"
                >
                  <Copy className="w-4 h-4" />
                </button>
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6">
              <div className="flex justify-between items-start mb-4">
                <h4 className="text-purple-400 font-bold text-lg">📊 Pitch Deck Outline</h4>
                <button
              ...
              <div className="space-y-3">
                {companyResult.pitchDeckOutline.map((slide, index) => (
                  <div key={index} className="flex items-start space-x-3 bg-purple-500/10 border border-purple-500/20 rounded-lg p-3">
                    ...
                    <p className="text-gray-300 text-sm">{slide}</p>
                  </div>
                ))}
              </div>
            </div>
