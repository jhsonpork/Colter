...
// Create a mock IdeaToCompany result
const createMockIdeaToCompany = (businessIdea: string) => {
  return {
...
export const callGeminiAPI = async (prompt: string): Promise<any> => {
  ...
  } else if (prompt.includes('Transform this business idea into a complete company blueprint')) {
    const ideaMatch = prompt.match(/Business Idea: "([^"]*)"/);
    return createMockIdeaToCompany(ideaMatch ? ideaMatch[1] : "business idea");
  }
