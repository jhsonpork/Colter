...
const createMockModularAd = (hook: string, pain: string, solution: string, cta: string) => ({
  components: { hook: hook||"⚠️ Mock hook", pain: pain||"⚠️ Mock pain point", solution: solution||"⚠️ Mock solution", cta: cta||"⚠️ Mock CTA" },
  assembledAds: [
    { version: "Standard Flow", fullAd: `⚠️ ${hook||"Mock hook"} ${pain||"Mock pain"} ${solution||"Mock solution"} ${cta||"Mock CTA"}`, effectiveness: 7 },
    { version: "Problem-First", fullAd: `⚠️ ${pain||"Mock pain"} ${hook||"Mock hook"} ${solution||"Mock solution"} ${cta||"Mock CTA"}`, effectiveness: 6 },
    { version: "Solution-Led", fullAd: `⚠️ ${solution||"Mock solution"} ${pain||"Mock pain"} ${hook||"Mock hook"} ${cta||"Mock CTA"}`, effectiveness: 5 }
  ],
  templates: ["⚠️ [HOOK] → [PAIN] → [SOLUTION] → [CTA]","⚠️ [PAIN] → [HOOK] → [SOLUTION] → [CTA]","⚠️ [SOLUTION] → [PAIN] → [HOOK] → [CTA]"],
...
const createMockHyperPersona = (targetAudience: string) => ({
  ...
  psychologicalDrivers: [
    { driver: "⚠️ Mock Psychological Driver 1", explanation: "This is a mock explanation of this psychological driver." },
    { driver: "⚠️ Mock Psychological Driver 2", explanation: "This is a mock explanation of this psychological driver." },
    { driver: "⚠️ Mock Psychological Driver 3", explanation: "This is a mock explanation of this psychological driver." }
  ],
  decisionPatterns: [
    { pattern: "⚠️ Mock Decision Pattern 1", explanation: "This is a mock explanation of this decision pattern.", marketingImplications: "These are mock marketing implications." },
    { pattern: "⚠️ Mock Decision Pattern 2", explanation: "This is a mock explanation of this decision pattern.", marketingImplications: "These are mock marketing implications." }
  ],
  communicationStrategies: {
    messagingApproaches: ["⚠️ Mock messaging approach 1","⚠️ Mock messaging approach 2","⚠️ Mock messaging approach 3"],
    channelPreferences: ["⚠️ Mock channel preference 1","⚠️ Mock channel preference 2","⚠️ Mock channel preference 3"]
  },
  objectionHandling: [
    { objection: "⚠️ Mock Objection 1", psychologicalRoot: "This is a mock psychological root of the objection.", recommendedResponse: "This is a mock recommended response." },
    { objection: "⚠️ Mock Objection 2", psychologicalRoot: "This is a mock psychological root of the objection.", recommendedResponse: "This is a mock recommended response." }
  ]
});
...
const createMockCourseCurriculum = (topic: string) => ({
  ...
  bonusUnits: {
    slidePrompts: ["⚠️ Mock slide prompt #1. Please try again soon!","⚠️ Mock slide prompt #2. Our AI is busy right now.","⚠️ Mock slide prompt #3. We'll be back shortly!"],
    videoSegments: ["⚠️ Mock video segment #1. Please try again soon!","⚠️ Mock video segment #2. Our AI is busy right now.","⚠️ Mock video segment #3. We'll be back shortly!"],
...
export const generateAd = async (
  ...
  const inputContext = inputMode === 'description' ? 'Business Description' : 'Detailed Business Information';
...
export const callGeminiAPI = async (prompt: string): Promise<any> => {
  for (let i = 0; i < GEMINI_API_KEYS.length; i++) {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`Key ${apiKey.slice(0,5)} failed: ${response.status}`);
        continue;
      }
      ...
      if (!generatedText) {
        console.warn(`Key ${apiKey.slice(0,5)} returned no text.`);
        continue;
      }

      // Parse JSON if present
      const jsonMatch = generatedText.match(/\{[\s\S]*\}|\[[\s\S]*\]/);
      return jsonMatch ? JSON.parse(jsonMatch[0]) : generatedText;
    } catch (err) {
      console.warn(`Key ${apiKey.slice(0,5)} threw error:`, err);
      continue;
    }
  }

  console.error('All Gemini API keys failed; returning mock.');
  // Fallback based on prompt
  if (prompt.includes('Create a 7-day viral ad campaign')) return createMockCampaignDays();
  if (prompt.includes('viral ad package')) return createMockAdResult();
  if (prompt.includes('Rewrite this ad')) return `⚠️ Our AI is currently busy. Please try again later.`;
  if (prompt.includes('Take this trending topic')) {
    const trend = prompt.match(/Trending Topic: "([^"]*)"/)?.[1] || '';
    const niche = prompt.match(/User's Niche: "([^"]*)"/)?.[1] || '';
    return createMockTrendRewrite(trend, niche);
  }
  if (prompt.includes('Create cold email')) {
    const type = prompt.match(/Email Type: "([^"]*)"/)?.[1] || 'outreach';
    return createMockEmailResult(type);
  }
  if (prompt.includes('viral hooks, scripts, hashtags')) {
    const plat = prompt.match(/Platform: ([^\n]*)/)?.[1] || '';
    const ct = prompt.match(/Content Type: ([^\n]*)/)?.[1] || '';
    return createMockSocialContent(plat, ct);
  }
  if (prompt.includes('Combine these components')) {
    const h = prompt.match(/Hook: "([^"]*)"/)?.[1] || '';
    const p = prompt.match(/Pain: "([^"]*)"/)?.[1] || '';
    const s = prompt.match(/Solution: "([^"]*)"/)?.[1] || '';
    const c = prompt.match(/CTA: "([^"]*)"/)?.[1] || '';
    return createMockModularAd(h,p,s,c);
  }
  if (prompt.includes('hyper-detailed persona')) {
    const aud = prompt.match(/Target Audience: "([^"]*)"/)?.[1] || '';
    return createMockHyperPersona(aud);
  }
  if (prompt.includes('complete funnel')) {
    const handle = prompt.match(/Account Handle: "([^"]*)"/)?.[1] || '';
    return createMockCreatorFunnel(handle);
  }
  if (prompt.includes('Extract and expand comments')) {
    const post = prompt.match(/Viral Post: "([^"]*)"/)?.[1] || '';
    return createMockCommentExploder(post);
  }
  if (prompt.includes('Predict optimal posting times')) {
    const ct = prompt.match(/Content Type: "([^"]*)"/)?.[1] || '';
    const tz = prompt.match(/Timezone: "([^"]*)"/)?.[1] || '';
    const ni = prompt.match(/Niche: "([^"]*)"/)?.[1] || '';
    return createMockTimingForecaster(ct, tz, ni);
  }
  if (prompt.includes('random ad styles')) {
    const bt = prompt.match(/Business Type: "([^"]*)"/)?.[1] || '';
    return createMockStyleRoulette(bt);
  }
  if (prompt.includes('complete curriculum')) {
    const tp = prompt.match(/Topic: "([^"]*)"/)?.[1] || '';
    return createMockCourseCurriculum(tp);
  }

  // generic fallback
  return {
    message: "⚠️ Our AI is busy. Please try again later.",
    mockResponse: true,
